...
-V $inputVcf \
--filterName "snpQD" \
--filterExpression "vc.hasAttribute('QD') && QD < 6.0" \
--filterName "snpMQ" \
--filterExpression "vc.hasAttribute('MQ') && MQ < 60.0" \
--filterName "snpMQRankSum" \
--filterExpression "vc.hasAttribute('MQRankSum') && MQRankSum < -1" \
--filterName "snpFS" \
--filterExpression "vc.hasAttribute('FS') && FS > 20.0" \
--filterName "snpSOR" \
--filterExpression "vc.hasAttribute('SOR') && SOR > 2.0" \
--filterName "snpReadPosRankSum" \
--filterExpression "vc.hasAttribute('ReadPosRankSum') && ReadPosRankSum < -1.0" \
--filterName "snpDP" \
